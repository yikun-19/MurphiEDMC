...
void ReportManager::print_progress(void)
{
  ...
  if (args->print_progress.value && StateSet->NumElts() % args->progress_count.value == 0) {
    ...
    cout << "\t" << StateSet->NumElts() << " states explored in " << SecondsSinceStart() << "s, with " << Rules->NumRulesFired() << " rules fired and " << StateSet->QueueNumElts() << " states in the queue.\n";
    cout.flush();
  }
}
...
void ReportManager::print_progress_Ls(void)
{
  ...
  // pring progress report every <args->progress_count> new states found
  if (args->print_progress.value && (StateSet_Ls->NumElts_LS() % args->progress_count.value) == 0) {
    if (!initialized) {
      cout << "\nProgress Report:\n\n";
      initialized = TRUE;
    }
    cout << "\t" << StateSet_Ls->NumElts_LS() << " states explored in " << SecondsSinceStart() << "s, with " << Rules->NumRulesFired() << " rules fired and " << StateSet_Ls->QueueNumElts() << " states in the queue.\n";
    cout.flush();
  }
}

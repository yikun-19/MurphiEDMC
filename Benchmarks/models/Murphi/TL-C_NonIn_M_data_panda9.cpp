...
if ( !(!(mu__boolexpr653)) ) Error.Error("Assertion failed: test.m, line 713.");
mu_Tree[mu_i].mu_reqId = mu_b;
mu_msg.mu_opcode = mu_Probe;
mu_msg.mu_para = mu_toN;
mu_msg.mu_addr = mu_a;
...
void Goal_value() {  // makun: set goalstate
    ...
    // set goalstate value: 
// set states
mu_Tree[1].mu_cache.mu_line[1].mu_state.value(mu_Branch);
mu_Tree[2].mu_cache.mu_line[1].mu_state.value(mu_Branch);
mu_Tree[4].mu_cache.mu_line[1].mu_state.value(mu_None);
mu_Tree[5].mu_cache.mu_line[1].mu_state.value(mu_Branch);
mu_Tree[3].mu_cache.mu_line[1].mu_state.value(mu_None);
mu_Tree[6].mu_cache.mu_line[1].mu_state.value(mu_Branch);
mu_Tree[7].mu_cache.mu_line[1].mu_state.value(mu_None);
// set states
...
// mu_Tree[7].mu_master_pending[1].value(mu_pending_Grant);
    ...
    carelist.clear(); 
    carelist.push_back("Tree[1].cache.line[1].state");

    // set carelist about rulebase 
    carelist_rb.clear();  
    carelist_rb.push_back("Tree[2].cache.line[1].state");
    carelist_rb.push_back("Tree[3].cache.line[1].state");

    carelist_3.clear();  
    carelist_3.push_back("Tree[4].cache.line[1].state");
    carelist_3.push_back("Tree[5].cache.line[1].state");
    carelist_3.push_back("Tree[6].cache.line[1].state");
    carelist_3.push_back("Tree[7].cache.line[1].state");

    carelist_flow.clear(); 
    carelist_flow.push_back("Tree[4].master_pending[1]");
    carelist_flow.push_back("Tree[5].master_pending[1]");
    carelist_flow.push_back("Tree[6].master_pending[1]");
    carelist_flow.push_back("Tree[7].master_pending[1]");
    carelist_flow.push_back("Tree[4].cache.valid[1]");
    carelist_flow.push_back("Tree[5].cache.valid[1]");
    carelist_flow.push_back("Tree[6].cache.valid[1]");
    carelist_flow.push_back("Tree[7].cache.valid[1]");
...
{"test_reachable_state_119", &mu__condition_1744, NULL, },
};// set invariants

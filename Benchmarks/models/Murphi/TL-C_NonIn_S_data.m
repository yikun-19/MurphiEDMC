...
begin
    ...
    Tree[j].chanC[b].opcode  := ProbeAckData;
    Tree[j].chanC[b].data    := Tree[i].cache.data;
    Tree[j].chanC[b].dirty   := Tree[i].cache.dirty;
...
begin
    ...
    Tree[i].cache.state := Tip;
    Tree[i].cache.data := Tree[i].chanC[b].data;
    Tree[i].cache.dirty := Tree[i].chanC[b].dirty;
    undefine Tree[i].chanC[b];
    undefine Tree[i].slave_pending[b];
endrule;
...
-- forall i: nodeId do
--     (Tree[i].cache.state = Tip | Tree[i].cache.state = Branch) -> Tree[i].cache.data = auxData
-- endforall;

    ...
    if (!declared) {
       ...
       the declaration of the object in the generated code. */
        ...
        fprintf(codefile,
                ...
                mu_type, mu_name, mu_type, mu_name, mu_type, accuracy, exponent_value,
                numbits,    //IM: fixed, exponent_value instead of exponent
        ...
        switch (indextype->gettypeclass()) {
            ...
            case typedecl::Scalarset:
                fprintf(codefile,
                        ...
                        indextype->getright(),    /* upper bound */
                        indextype->getleft(),    /* index adjust (3rd line) */
                        indextype->name, indextype->getleft(), indextype->getright(),
                        indextype->getleft());    /* index adjust (after #else) */
                break;
            case typedecl::Union:
                (void)
...
/********************
  code for expr
 ********************/
const char *expr::generate_code() {  // makun: return a tmp node, and fprintf() when calling lower generate_code()
    if (constval)
    ...
    if (quantindex >= numquants) {
        ...
        if (therule->condition != NULL) {
            ...
            fprintf(codefile, "bool %s() // Condition for Rule \"%s%s\"\n" "{\n" "  return %s(%s );\n" "}\n\n",
                    condname, therule->name, namequants, therule->condname,
        ...
        if (therule->body != NULL) {
            ...
            fprintf(codefile, "void %s() // Action for Rule \"%s%s\"\n" "{\n" "  %s(%s );\n" "};\n\n", rulename,
                    therule->name, namequants, therule->rulename,
                    quantactuals + 1);    /* + 1 to skip the leading comma. */
        } else
            rulename = NULL;
...
std::vector<std::string> extract_variables(const std::string& expression) {

    std::regex pattern_tmp(R"(\[[\w()%/\-\s\+]+\])");
    std::string expression_simple = std::regex_replace(expression, pattern_tmp, "[node]");

    std::regex pattern(R"([0-9_.\[\]a-zA-Z]+)"); // '\b' is not working
    std::sregex_iterator begin(expression_simple.begin(), expression_simple.end(), pattern);
...
std::string removeSubstring(const std::string& original, const std::string& toRemove, const std::string& replacement) {
    std::string result = original;
    size_t pos = result.find(toRemove);

    while (pos != std::string::npos) {
        result.replace(pos, toRemove.length(), replacement);
        pos = result.find(toRemove, pos + replacement.length());
    }

    return result;
}

std::map<std::string, std::string> parseSpecificInvariant(const std::string &expr) {

    std::map<std::string, std::string> result;
    std::string expr_backup = expr;
    trim(expr_backup);

    std::regex pattern1(R"(!\(([a-zA-Z0-9_&|!\[\]\.\(\)=]+)\))");
    std::smatch matches1;
    if (std::regex_search(expr_backup, matches1, pattern1)) {
        std::string equals = matches1[1].str();
        std::vector<std::string> equal_list = split(equals, '&');

        for (const auto &s : equal_list) {
            std::regex pattern2(R"(([a-zA-Z0-9_\[\]\.\(\)]+)==([a-zA-Z0-9_\[\]\.\(\)]+))");
            std::smatch matches2;
            if (std::regex_search(s, matches2, pattern2)) {
                std::string attr = matches2[1].str();
                std::string val = matches2[2].str();
                result[attr] = val;
            } else {
                fprintf(codefile, "    // Note: This equal is not handled: %s . \n", s.c_str());
            }
        }
    } else {
        fprintf(codefile, "    // Note: Inv is not the excepted format. \n");
    }

    return result;
}

bool checkVarValid(const std::string & v) {
    if (v == "" || v == "true" || v == "false") {
        return false;
    }
    if (v == "if" || v == "while" || v == "for") {
        return false;
    }
...
void generate_carelists() {
    simplerule *sr;
    int rule_no = 0;
    for (sr = simplerule::SimpleRuleList; sr != NULL; sr = sr->NextSimpleRule) {
        if (sr->getclass() == rule::Simple && sr != error_rule) {

            fprintf(codefile, "    // RuleName: %s \n", sr->name);
            rule_no ++;
            fprintf(codefile, "    rule_info rule_tmp_%d; \n", rule_no);

            std::string r_guard = sr->condition->get_string();
            std::vector<std::string> r_guard_variables = extract_variables(r_guard);

            std::string r_guard_string = "";
            for (auto& var : r_guard_variables) {
                var = removeSubstring(var, "mu_", "");
                if (checkVarValid(var))
                    r_guard_string += '\"' + var + '\"' + ", ";
            }
            fprintf(codefile, "    rule_tmp_%d.guard = { %s }; \n", rule_no, r_guard_string.c_str());

            std::string stmt_str = "";
            for (stmt *b = sr->body; b != NULL; b = b->next)
                stmt_str += b->get_string();
            std::string r_action = stmt_str;
            std::vector<std::string> r_action_variables = extract_variables(r_action);
            std::string r_action_string = "";
            for (auto& var : r_action_variables) {
                var = removeSubstring(var, "mu_", "");
                if (checkVarValid(var))
                    r_action_string += '\"' + var + '\"' + ", ";
            }
            fprintf(codefile, "    rule_tmp_%d.action = { %s }; \n", rule_no, r_action_string.c_str());
            fprintf(codefile, "    rules.push_back(rule_tmp_%d); \n", rule_no);
        }
    }
}

void generate_goalstate() {
    simplerule *sr;
    std::string inv_test = "None";
    for (sr = simplerule::SimpleRuleList; sr != NULL; sr = sr->NextSimpleRule) {
        if (sr->getclass() == rule::Invar) {
            inv_test = sr->condition->get_string();
            break;
        }
    }
    fprintf(codefile, "\n    // inv: %s\n", inv_test.c_str());
    fprintf(codefile, "    vector<std::string> carelist_tmp; \n");
    fprintf(codefile, "    carelists.push_back(carelist_tmp); \n");

    std::map<std::string, std::string> inv_map = parseSpecificInvariant(inv_test);
    for (auto &pair : inv_map) {
        std::string attr_str = removeSubstring(pair.first, "mu_", "");
        attr_str = removeSubstring(attr_str, "(", "");
        attr_str = removeSubstring(attr_str, ")", "");
        std::string attr_mu_str = removeSubstring(pair.first, "(", "");
        attr_mu_str = removeSubstring(attr_mu_str, ")", "");
        std::string val_str = removeSubstring(pair.second, "(", "");
        val_str = removeSubstring(val_str, ")", "");

        fprintf(codefile, "    funcMap[\"%s\"] = []() -> int { return %s; }; \n", attr_str.c_str(), attr_mu_str.c_str());
        fprintf(codefile, "    goalstateMap[\"%s\"] = %s; \n", attr_str.c_str(), val_str.c_str());
        fprintf(codefile, "    carelists[0].push_back(\"%s\"); \n", attr_str.c_str());
    }
}

int generate_heuristics() {
...
bool checkFuncAttr(string attr_name)
{
    ...
    fprintf(codefile, "void initHeuristics() {\n");
...
const char *program::generate_code() {
    ...
    if (args->bpctl_prob_ord && count == 1)
        printf("Warning: Since there is only one rule, there is no need to sort probabilities\n");

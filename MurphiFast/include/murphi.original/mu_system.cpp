...
/* StateManager */
/************************************************************/
StateManager::StateManager(int mode, unsigned long NumStates) : NumStates(NumStates), statesCurrentLevel(0),
                                                                statesNextLevel(0), currentLevel(0), pno(1.0) {
#if __WORDSIZE == 64
    if (!args->print_64bit_info.value) {
        ...
        if (mode == 0) {
            ...
            queue = new state_priority_queue((unsigned long) (gPercentActiveStates * NumStates));
        }
...
bool StateManager::IsPresent(state *s, bool valid, bool permanent) {
    return (the_states->was_present(s, valid, permanent));
}
...
bool StateManager::QueueEnqueue(state* s) {

    // nextstate -> table[h]
    the_states->was_present(s, TRUE, TRUE);
...
void StateManager::print_capacity() {
    if (args->main_alg.mode == argmain_alg::Verify_dfs || args->main_alg.mode == argmain_alg::Verify_bfs) {
        cout << "\nMemory usage:\n\n";
        cout << "\t* The size of each state is " << BITS_IN_WORLD << " bits " << "(rounded up to " << BLOCKS_IN_WORLD
             << " bytes).\n";
...
{
    cout << "start to print_trace_aux()" << endl;
    state original;
    char *s;
...
char *StartStateManager::StateName(StatePtr p) {
    state nextstate;
    if (!p.isStart())
        Error.Notrace("Internal: Cannot find startstate name for non startstate");
    for (what_startstate = 0; what_startstate < numstartstates; what_startstate++) {
        StartState();
    ...
    for (what_rule = 0; what_rule < numrules; what_rule++) {
        ...
        if (generator->Condition(what_rule)) {
            ...
            if (StateEquivalent(&nextstate, p)) {
                ...
                cout << "Rule " << (s = generator->Name(what_rule)) << " fired.\n";  // << rules[ what_rule ].name
                delete[]s;        // Uli: avoid memory leak
                if (fullstate)
...
AlgorithmManager::AlgorithmManager() {
    ...
    switch (args->main_alg.mode) {
        case argmain_alg::Verify_bfs:
            StateSet = new StateManager(0, NumStatesGivenBytes(args->mem.value));
            StateSet->print_capacity();
            break;
        case argmain_alg::Verify_dfs:
            StateSet = new StateManager(1, NumStatesGivenBytes(args->mem.value));
            StateSet->print_capacity();
            break;
        case argmain_alg::Verify_edmc:
            StateSet = new StateManager(2, NumStatesGivenBytes(args->mem.value));
            StateSet->print_capacity();
            break;
        case argmain_alg::Verify_ls:
            ...
            StateSet->print_capacity();
            break;
        case argmain_alg::Verify_idastar:
            StateSet = new StateManager(2, NumStatesGivenBytes(args->mem.value));
            StateSet->print_capacity();
            break;

...
public:
    StateManager(int mode, unsigned long NumStates);

    ~StateManager();

    bool Add(state *s, bool valid, bool permanent);

    bool IsPresent(state *s, bool valid, bool permanent);

    bool QueueIsEmpty();

    state *QueueTop();

    bool QueueEnqueue(state * s);
...
public:
    ...
    bool GetNextStatesByHeuristic();

    bool StatePropagate();

    bool GetNextStatesNoHash();
...
#include <vector>
vector<vector<std::string>> carelists;
void initHeuristics();

class rule_info {
public:
    vector<std::string> guard;

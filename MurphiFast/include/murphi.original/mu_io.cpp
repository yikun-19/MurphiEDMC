...
#define VERIFY_EDMC_FLAG "-edmc"
#define VERIFY_LS_FLAG "-ls"
#define VERIFY_IDAStar_FLAG "-idastar"
...
void argclass::ProcessOptions(string_iterator *options) {
    ...
    for (options->start(); !options->done(); options->next()) {
        ...
        if (strcmp(option, VERIFY_EDMC_FLAG) == 0) {
            main_alg.set(argmain_alg::Verify_edmc);
            continue;
        }
        ...
        if (strcmp(option, VERIFY_IDAStar_FLAG) == 0) {
            main_alg.set(argmain_alg::Verify_idastar);
            continue;
        }
